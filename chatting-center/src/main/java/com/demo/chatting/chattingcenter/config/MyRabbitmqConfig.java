package com.demo.chatting.chattingcenter.config;
...
public class MyRabbitmqConfig {
    ...
    @Bean
    public Binding directBindReceiveA(
            @Qualifier("directExchange") Exchange exchange,
            @Qualifier("receiveQueueA") Queue queue
    ) {
        return BindingBuilder.bind(queue).to(exchange).with("receiveA").noargs();
    }


    @Bean
    public Binding directBindReceiveB(
            @Qualifier("directExchange") Exchange exchange,
            @Qualifier("receiveQueueB") Queue queue
    ) {
        return BindingBuilder.bind(queue).to(exchange).with("receiveB").noargs();
    }
